...
public partial class FileProcessor :  ContentPage, INotifyPropertyChanged
{
    ...
    private async Task ProcessFileAsync()
    {
        ...
        try
        {
            ...
            var manifest = PdfProcessor.ConvertPdfToExcel($"{outputSimplifiedFilePath}.pdf", $"{outputSimplifiedFilePath}.xlsx");

            if (manifest == null)
            {
                info.Add("❌ Failed to convert PDF to Excel.");
                OutputInfo = string.Join("\n", info);
                GlobalLogger.LogError("Failed to convert manifest PDF into Excel");
                return;
            }
            else
            {
            ...
            var receiptXmlPath = Path.Combine(scaleInterfaceFolderOutput, $"PER-CO-FTG_Receipt-FTG-{manifest.GetManifestDateString()}.rcxml");
            ...
            var shipmentXmlPath = Path.Combine(scaleInterfaceFolderOutput, $"PER-CO-FTG-Shipments-FTG-{manifest.GetManifestDateString()}.shxml");
    ...
    private async void OnOpenOutputClicked(object sender, EventArgs e)
    {
        ...
        catch (Exception ex)
        {
            try
            {
                ...
                {
                    Process.Start("open", OutputFolderPath);
                }
                else
                {
