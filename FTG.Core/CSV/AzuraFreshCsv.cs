...
{
    public class FlexibleDateOnlyConverter : DefaultTypeConverter
    {
        ...
        public override object ConvertFromString(string text, IReaderRow row, MemberMapData memberMapData)
        {
            ...
            {
                if (DateTime.TryParseExact(text, format, CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime dateTime))
                {
    ...
    /// <returns>A FreshToGoManifest object containing the processed orders and metadata, or null if the file is invalid or an error occurs during processing.</returns>
    ...
    private static List<FreshToGoOrder> CreateOrdersFromRecords(List<AzuraFreshCsvRecord> records)
    {
        ...
        foreach (var record in records)
        {
            var order = new FreshToGoOrder(
                ...
                record.CustomerPoNo != String.Empty ? record.CustomerPoNo : record.OrderNumber,
                record.PurchaseOrderNo != String.Empty ? record.PurchaseOrderNo : record.OrderNumber, // Assuming InvoiceNo is used as OrderNumber
                record.InvoiceNo != String.Empty ? record.InvoiceNo : record.PurchaseOrderNo, // Assuming InvoiceNo is used as InventoryNumber
                record.UnitQty,
                record.CrateQty
            );
